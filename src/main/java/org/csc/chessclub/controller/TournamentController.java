...
public class TournamentController {
  ...
  public ResponseEntity<ResponseDto<PageResponseDto<TournamentDto>>> getAllPagedTournaments(
    ...
    return ResponseEntity.status(HttpStatus.OK)
        .body(
            new ResponseDto<>(
                pageUtils.populatePageResponseDto(pagedTournaments), LIST_FOUND, true));
  }
  ...
  public ResponseEntity<ResponseDto<UUID>> deleteTournament(@ValidUUID @PathVariable UUID uuid) {
    ...
    return ResponseEntity.status(HttpStatus.OK).body(new ResponseDto<>(uuid, DELETED, true));
  }

  @GetMapping("?eventUuid={eventUuid}")
  public ResponseEntity<ResponseDto<PageResponseDto<TournamentDto>>>
      getAllPagedTournamentsByEventUuid(
          @ValidUUID @PathVariable UUID eventUuid, @PageableDefault Pageable pageable) {
