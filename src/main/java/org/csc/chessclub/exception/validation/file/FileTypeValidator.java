...
public class FileTypeValidator implements ConstraintValidator<ValidFile, MultipartFile> {
   ...
   * validation to fail, in case the file is null or empty, this logic should be modified to return
   * false or a message: 'if (file == null || file.isEmpty()) { return true; }'.
   */
  @Override
  public boolean isValid(MultipartFile file, ConstraintValidatorContext context) {
    ...
    try {
      ...
      if (!allowedMimes.isEmpty()
          && allowedMimes.stream().noneMatch(detectedType::equalsIgnoreCase)) {
