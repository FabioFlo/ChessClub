...
public class FileTypeValidator implements ConstraintValidator<ValidFile, MultipartFile> {
    ...
    /** By default, this annotation does not throw an exception if the file is null.
     * If you want the validation to fail,
     * in case the file is null or empty, this logic should be modified in:
     * 'if (file == null || file.isEmpty()) {
     *             return true;
     *         }'
     * to return false or a message.*/
    @Override
    public boolean isValid(MultipartFile file, ConstraintValidatorContext context) {
        ...
        if (filename == null) messages.add(FileValidationMessage.NO_FILENAME);
        ...
        try {
            ...
            if (!allowedMimes.isEmpty() && allowedMimes.stream().noneMatch(detectedType::equalsIgnoreCase)) {
