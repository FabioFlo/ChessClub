...
public class UserServiceImpl implements UserService {
  ...
  public UserDto create(RegisterUserRequest request) {
    Optional<UserEntity> existingUser =
        userRepository.findUserEntityByUsernameOrEmail(request.username(), request.email());
  ...
  public UserDto update(UpdateUserRequest userRequest) {
    ...
    Optional<UserEntity> existingUser =
        userRepository.findByUsernameOrEmailAndUuidNot(
            userRequest.username(), userRequest.email(), userRequest.uuid());
    if (existingUser.isPresent()) {
      throw new UserServiceException("Email or username already taken");
    }
    UserEntity user =
        userRepository
            .findById(userRequest.uuid())
            .orElseThrow(
                () ->
  ...
  public Role updateUserRole(UpdateRoleDto updateRoleDto) {
    int result =
        userRepository.updateRole(
            updateRoleDto.uuid(), updateRoleDto.oldRole(), updateRoleDto.newRole());
    if (result == 0) {
  ...
  public void updateUserPassword(UpdatePasswordDto updatePasswordDto) {
    int result =
        userRepository.updatePassword(
            updatePasswordDto.uuid(), passwordEncoder.encode(updatePasswordDto.password()));
