...
public class GameMapperTests {
  ...
  void shouldMapGameEntityToGameDto() {
    ...
    assertEquals(gameDto.uuid(), game.getUuid(), "Uuid should match");
    assertEquals(
        gameDto.blackPlayerName(), game.getBlackPlayerName(), "Black player name should match");
    assertEquals(
        gameDto.whitePlayerName(), game.getWhitePlayerName(), "White player name should match");
    assertEquals(gameDto.result(), game.getResult(), "Result should match");
    assertEquals(gameDto.pgn(), game.getPgn(), "Pgn should match");
  }
  ...
  void shouldMapCreateGameDtoToGameEntity() {
    ...
    assertEquals(
        game.getWhitePlayerName(), gameDto.whitePlayerName(), "White player name should match");
    assertEquals(
        game.getBlackPlayerName(), gameDto.blackPlayerName(), "Black player name should match");
    assertEquals(game.getResult(), gameDto.result(), "Result should match");
    assertEquals(game.getPgn(), gameDto.pgn(), "Pgn should match");
  }
  ...
  void shouldMapUpdateGameDtoToGameEntity() {
    UpdateGameDto gameDto =
        new UpdateGameDto(gameId, whitePlayerName, blackPlayerName, pgn, result, null);
    GameEntity game =
        gameMapper.updateGameDtoToGame(gameDto, GameEntity.builder().uuid(gameId).build());

    assertEquals(game.getUuid(), gameDto.uuid(), "Uuid should match");
    assertEquals(
        game.getWhitePlayerName(), gameDto.whitePlayerName(), "White player name should match");
    assertEquals(
        game.getBlackPlayerName(), gameDto.blackPlayerName(), "Black player name should match");
  ...
  void shouldMapListGameEntityToGameDto() {
    ...
    assertEquals(2, gameDtos.size(), "List size size should match");
    assertEquals(game.getUuid(), gameDtos.getFirst().uuid(), "Uuid should match");
    assertEquals(
        game.getWhitePlayerName(),
        gameDtos.getFirst().whitePlayerName(),
        "White player name should match");
    assertEquals(
        game.getBlackPlayerName(),
        gameDtos.getFirst().blackPlayerName(),
        "Black player name should match");
    assertEquals(game.getResult(), gameDtos.getFirst().result(), "Result should match");
    assertEquals(game.getPgn(), gameDtos.getFirst().pgn(), "Pgn should match");
  }
  ...
  void shouldMapPageGameEntityToGameDto() {
    ...
    assertAll(
        "Page mapper assertions",
        () -> assertEquals(1, gamesDto.getTotalElements(), "Total elements should match"),
        () ->
            assertEquals(
                game.getUuid(), gamesDto.getContent().getFirst().uuid(), "Uuid should match"),
        () ->
            assertEquals(
                game.getWhitePlayerName(),
                gamesDto.getContent().getFirst().whitePlayerName(),
                "White player name should match"),
        () ->
            assertEquals(
                game.getBlackPlayerName(),
                gamesDto.getContent().getFirst().blackPlayerName(),
                "Black player name should match"),
        () ->
            assertEquals(
                game.getResult(), gamesDto.getContent().getFirst().result(), "Result should match"),
        () ->
            assertEquals(game.getPgn(), games.getContent().getFirst().getPgn(), "Png should match"),
