...
public class EventMapperTests {
  ...
  void shouldMapEventToEventDtoCorrectly() {
    ...
    assertEquals(event.getUuid(), eventDto.uuid(), "UUID should be equal");
    assertEquals(event.getDescription(), eventDto.description(), "Description should be equal");
    assertEquals(
        event.getAnnouncementPDF(), eventDto.announcementPDF(), "Announcement PDF should be equal");
    assertEquals(event.getAuthor(), eventDto.author(), "Author should be equal");
    assertEquals(event.getCreatedAt(), eventDto.createdAt(), "Created At should be equal");
    assertEquals(event.getTitle(), eventDto.title(), "Title should be equal");
  }
  ...
  void shouldMapCreateEventDtoToEventCorrectly() {
    ...
    assertEquals(eventDto.title(), event.getTitle(), "Title should be equal");
    assertEquals(eventDto.description(), event.getDescription(), "Description should be equal");
    assertEquals(eventDto.author(), event.getAuthor(), "Author should be equal");
  }
  ...
  void shouldMapListOfEventEntityToListOfEventDtoCorrectly() {
    List<EventDto> eventDtoList =
        eventMapper.eventEntityListToEventDtoList(java.util.List.of(event));

    assertEquals(1, eventDtoList.size(), "List should contain only one element");
    assertEquals(event.getUuid(), eventDtoList.getFirst().uuid(), "UUID should be equal");
    assertEquals(
        event.getDescription(),
        eventDtoList.getFirst().description(),
        "Description should be equal");
    assertEquals(
        event.getAnnouncementPDF(),
        eventDtoList.getFirst().announcementPDF(),
  ...
  void shouldMapUpdateEventDtoToEventEntityCorrectly() {
    ...
    EventEntity updatedEvent =
        eventMapper.updateEventDtoToEvent(
            new UpdateEventDto(uuid, updatedTitle, DESCRIPTION, AUTHOR), event);
  ...
  void testMapPageOfEventEntityToPageOfEventDto() {
    ...
    Page<EventEntity> pageOfTournament =
