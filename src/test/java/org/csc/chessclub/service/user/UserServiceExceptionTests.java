...
public class UserServiceExceptionTests {
  ...
  void testCreateUser_whenUserProvided_shouldThrowUserExceptionIfEmailOrUsernameAlreadyTaken() {
    ...
    when(userRepository.findUserEntityByUsernameOrEmail(USERNAME, EMAIL))
        .thenReturn(Optional.ofNullable(user));

    UserServiceException exception =
        assertThrows(UserServiceException.class, () -> userService.create(userRequest));
  ...
  void
      testUpdateUser_whenUserProvided_shouldThrowUserExceptionIfEmailOrUsernameAlreadyTakenWithDifferentId() {
    ...
    when(userRepository.findByUsernameOrEmailAndUuidNot(USERNAME, EMAIL, user.getUuid()))
        .thenReturn(Optional.ofNullable(user));
    UpdateUserRequest updateUser = new UpdateUserRequest(uuid, USERNAME, EMAIL);

    UserServiceException exception =
        assertThrows(UserServiceException.class, () -> userService.update(updateUser));
  ...
  void testUserService_whenUserNotFoundById_shouldThrowCustomNotFoundException() {
    ...
    CustomNotFoundException exception =
        assertThrows(CustomNotFoundException.class, () -> userService.update(updateUser));

    assertTrue(
        exception.getMessage().contains(NotFoundMessage.USER_WITH_UUID.format((user.getUuid()))));
  }
