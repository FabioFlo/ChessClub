...
public class EventRepositoryTests extends BaseTestConfiguration {
  ...
  void testFindEventByTitle_whenGivenTitleAndAvailableIsTrue_returnEventWithGivenTitle() {
    ...
    List<EventEntity> retrievedEvent =
        eventRepository.findEventEntityByTitleAndAvailableTrue(TITLE_1);

    assertAll(
        "Find by title assertions",
        () -> assertNotNull(retrievedEvent, "Event should not be null"),
        () -> assertTrue(retrievedEvent.getFirst().isAvailable(), "Event should be available"),
        () -> assertEquals(1, retrievedEvent.size(), "List should contain only one element"),
        () ->
            assertEquals(
                TITLE_1, retrievedEvent.getFirst().getTitle(), "Title of Event should be equal"));
  }
  ...
  void testFindEventByAuthor_whenGivenAuthor_returnEventWithGivenAuthor() {
    ...
    List<EventEntity> retrievedEvents =
        eventRepository.findEventEntityByAuthorAndAvailableTrue(AUTHOR);

    assertAll(
        "Find event by author assertions",
        () -> assertNotNull(retrievedEvents, "Event should not be null"),
        () -> assertEquals(1, retrievedEvents.size(), "List should contain only one element"),
        () -> assertTrue(retrievedEvents.getFirst().isAvailable(), "Event should be available"),
        () ->
            assertEquals(
                AUTHOR, retrievedEvents.getFirst().getAuthor(), "Author of Event should be equal"));
  }
  ...
  void testFindAllAvailableEvents_whenGivenPageable_returnEventPage() {
    ...
    assertAll(
        "Paged events assertions",
        () -> assertNotNull(result.getContent(), "Result should not be null"),
        () -> assertEquals(1, result.getTotalElements(), "One element should be found"),
