...
public class TournamentRepositoryTests extends BaseTestConfiguration {
  ...
  void testGetTournamentByTitle_whenTitleProvided_returnTournamentIfExist() {
    ...
    List<TournamentEntity> result =
        tournamentRepository.getTournamentEntitiesByTitleAndAvailableTrue(title);

    assertAll(
        "Get by title assertions",
        () -> assertNotNull(result, "Result should not be null"),
        () -> assertEquals(2, result.size(), "Should contain one tournament"),
        () ->
            assertTrue(
                result.stream().allMatch(TournamentEntity::isAvailable),
  ...
  void
      testGetAllTournaments_WhenEventUuidProvided_returnTournamentsWithEqualEventUuidAndAvailableTrue() {
    Pageable pageable = PageRequest.of(0, 10);
    Page<TournamentEntity> result =
        tournamentRepository.findTournamentEntitiesByEvent_UuidAndAvailableTrue(
    ...
    assertAll(
        ...
        () ->
            assertEquals(
                event.getUuid(),
                result.getContent().getFirst().getEvent().getUuid(),
