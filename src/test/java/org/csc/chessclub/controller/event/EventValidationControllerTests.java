...
public class EventValidationControllerTests extends BaseTestConfiguration {
  ...
  @Test
  @DisplayName(
      "Create Event - Should throw validation exception when invalid create event dto provided")
  void testCreateEvent_whenInvalidCreateEventDtoProvided_shouldThrowValidationException() {
    CreateEventDto invalidCreateEventDto = new CreateEventDto(null, DESCRIPTION, AUTHOR);

    ResponseDto<ValidErrorMessage> response =
        authHelper
  ...
  @Test
  @DisplayName(
      "Update Event - Should throw validation exception when invalid event details provided")
  ...
  void testUpdateEvent_whenFormatNotAllowed_thenValidationFails() {
    UpdateEventDto updEventDto =
        new UpdateEventDto(UUID.randomUUID(), "title", "description", "author");
