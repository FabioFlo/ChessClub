...
public class EventExceptionControllerTests extends BaseTestConfiguration {
    ...
    void testGetEvent_whenEventNotFound_shouldThrowCustomNotFoundExceptionFromGlobalExceptionHandler() {
        ...
        assertThat(response.data().message()).isEqualTo(
                NotFoundMessage.EVENT_WITH_UUID.getMessage().formatted(notFoundUuid));
    ...
    void testCreateEvent_whenUnauthenticatedUserTryToCreateEvent_shouldThrow401() {
        CreateEventDto createEventDto = new CreateEventDto("Test title", "Test description",
                "Test author");
        ResponseDto<ErrorMessage> response = given()
