...
public class EventExceptionControllerTests extends BaseTestConfiguration {
  ...
  void
      testGetEvent_whenEventNotFound_shouldThrowCustomNotFoundExceptionFromGlobalExceptionHandler() {
    ...
    assertThat(response.data().message())
        .isEqualTo(NotFoundMessage.EVENT_WITH_UUID.getMessage().formatted(notFoundUuid));
  ...
  void
      testGetEvent_whenInvalidUuid_shouldThrowCustomBadRequestExceptionFromGlobalExceptionHandler() {
    ResponseDto<ErrorMessage> response =
        given()
  ...
  void testCreateEvent_whenUnauthenticatedUserTryToCreateEvent_shouldThrow401() {
    CreateEventDto createEventDto =
        new CreateEventDto("Test title", "Test description", "Test author");
    ResponseDto<ErrorMessage> response =
        given()
    ...
    Assertions.assertThat(response).isNotNull().extracting(ResponseDto::success).isEqualTo(false);
