...
public class GameControllerTests extends BaseTestConfiguration {
  ...
  void testUpdateGame_whenUserAuthenticatedAndValidUpdateGameProvided_returnsUpdatedGame() {
    UpdateGameDto updateGameDto =
        new UpdateGameDto(gameId, whitePlayer, "Black player", "", Result.WhiteWon, null);
    ResponseDto<GameDto> response =
        authHelper
            .withBearerToken(userToken)
            .body(updateGameDto)
    ...
    assertThat(response).extracting(ResponseDto::message).isEqualTo(UPDATED);
  }
  ...
  void testGetGameById_whenExistingIdProvided_returnsGame() {
    ...
    assertThat(response).extracting(ResponseDto::data).extracting(GameDto::uuid).isEqualTo(gameId);
  }
  ...
  void testGetAll_whenGamesExists_returnsAllGames() {
    ResponseDto<PageResponseDto<GameDto>> response =
        given()
            .when()
            .get(apiPath)
            .then()
    ...
    assertThat(response)
        .extracting(ResponseDto::data)
        .extracting(PageResponseDto::pageSize)
        .isEqualTo(10);
